...
def updateResourceDef(url, user, pWd, resourceName, resJson):
    ...
    print("\tupdating resource for catalog:-" + url + " resource=" + resourceName + ' user=' + user)
...
def createResource(url, user, pWd, resourceName, resourceJson):
    ...
    return newResourceResp.status_code,json.loads(newResourceResp.text)
...
def uploadResourceFile(url, user, pWd, resourceName, fileName, fullPath):
    ...
    """
    print("uploading file for resource " + url + " resource=" + resourceName + ' user=' + user)
...
def executeResourceLoad(url, user, pWd, resourceName):
    ...
    print("starting scan resource " + url + " resource=" + resourceName + ' user=' + user)
...
def createOrUpdateAndExecuteResource(url, user, pwd, resourceName, templateFileName, fileName, inputFileFullPath, waitForComplete):
    '''
        ...
        if rc==200:
            ...
            if os.path.isfile(templateFileName):
                ...
                if createRc == 200:
                    validResource = True
                    '''
                    print("uploading file" + " " + filePath +fileName + " to resource: " + resourceName)
                    uploadRc = edcutils.uploadResourceFile(catalogServer, uid, pwd, resourceName, fileName, inputFile)
                    print(uploadRc)
                    '''
                else:
...
def createAttribute(url, user, pWd, attrJson):
    ...
    return newAttrResp.status_code,json.loads(newAttrResp.text)
